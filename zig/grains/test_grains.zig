...
test "grains on square 1" {
    const expected = 1;
    const actual = try comptime grains.square(1);
    try testing.expectEqual(expected, actual);
}

test "grains on square 2" {
    const expected = 2;
    const actual = try comptime grains.square(2);
    try testing.expectEqual(expected, actual);
}

test "grains on square 3" {
    const expected = 4;
    const actual = try comptime grains.square(3);
    try testing.expectEqual(expected, actual);
}

test "grains on square 4" {
    const expected = 8;
    const actual = try comptime grains.square(4);
    try testing.expectEqual(expected, actual);
}

test "grains on square 16" {
    const expected = 32_768;
    const actual = try comptime grains.square(16);
    try testing.expectEqual(expected, actual);
}

test "grains on square 32" {
    const expected = 2_147_483_648;
    const actual = try comptime grains.square(32);
    try testing.expectEqual(expected, actual);
}

test "grains on square 64" {
    const expected = 9_223_372_036_854_775_808;
    const actual = try comptime grains.square(64);
    try testing.expectEqual(expected, actual);
}
